...
module.exports = function (webmentions, url, aliases = []) {
  ...
  const potentialUrls = [
    ...
    ...aliases.filter(Boolean).map((a) => normaliseTrailingSlash(`https://${metadata.domain}${a}`)),
  ];
  ...
  // only allow webmentions that have an author name and a timestamp (if a comment)
  ...
  return webmentions
    .filter((entry) => potentialUrls.includes(normaliseTrailingSlash(entry["wm-target"])))
