...
let cursorChar =
  document.getElementsByTagName('html')[0].getAttribute('data-cursorchar') ||
  '❤︎';

export function setCursorChar(value) {
  cursorChar = value;
}
...
export function fairyDustCursor(options) {
  ...
  function init() {
    ...
    possibleColors.forEach((color) => {
      let measurements = context.measureText(cursorChar);
...
export function destroyCursor() {
  ...
  document.documentElement.removeAttribute('data-cursorchar');
  if (cursor) {
    cursor.remove();
  }
}
